<div class="py-2 col-span-6 sm:col-span-3 @if(in_array($atributo_id, $requeridos) && !$respuestas[$atributo_id]) border-l-4 border-red-600 px-2 bg-red-50 @endif">
    <label for="{{ $name }}" class="block text-sm @if(in_array($atributo_id, $requeridos) && !$respuestas[$atributo_id]) text-red-600 font-bold @elseif(in_array($atributo_id, $requeridos) && $respuestas[$atributo_id]) text-green-600 font-bold @else font-medium text-gray-700 @endif ">
    ...
    <select id="{{ $name }}" name="{{ $name }}" wire:model="respuestas.{{ $atributo_id }}" autocomplete="" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        <option></option>
